...
# Graph of GDP per capita Growth Rate in each Continent against Time
...
# Save the graph
ggsave(
  filename = "Graph of GDP per capita Growth Rate in each Continent against Time.png",
...
# Use line of best fit for easier comparison of general trend
...
# Plot graphs for GDP growth rate
...
Countries_Income_Group <- Countries_GDP_Growth_Rate %>%
  ...
  # Create column to sort countries by Income Group
...
# Plot graphs of GDP Growth Rate for each Income Group
...
# Graphs of mean GDP per capita Growth Rates in each Continent for each Income Group, with respective targets
...
# Create data frames of number of countries per Continent that met their GDP Growth Rate Targets per Year for each Income Group
...
# Create data frame with proportion of countries per Continent that met their GDP Growth Rate Targets per Year
...
# Graph of proportion of countries per Continent that met their GDP Growth Rate Target per Year
Graph_Proportion_Achieved_Growth <- ggplot(Proportion_Achieved_Growth) +
  aes(x = Year, y = Proportion, fill = Continent) +
  geom_col() +
  
  # Have a separate bar graph for each Continent
  ...
  # Increase font size of Continent facet label
...
# Find the latest year with Growth Rates and remove all terms with missing values
